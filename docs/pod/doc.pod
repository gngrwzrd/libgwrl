...
C and CMake and are willing to try and add support for your platform or compiler,
it'd be something worth contributing to GWRL.

I'm willing to help get GWRL compiling on other systems - you'd have to
dedicate some resources on a server over SSH with git, CMake, and a compiler
installed. I won't be able to drop everything and add support for a target
operating system or compiler though - I'd do it when I get free time,    unless
you want to pay me.
...
By default CMake is configured to create a static I<.a> archive and a I<dyld>
for Linux and Mac. On Windows CMake creates a I<lib> file and I<dll>.
...
The reactor is not intended to be used on Windows by itself. You should still read
...
Create a reactor with:

 gwrl * gwrl_create();

Run a reactor with these:

 void gwrl_run_once(gwrl * rl);
 void gwrl_run(gwrl * rl);

If you use gwrl_run() the reactor will run indefinitely until it's stopped.
If you use gwrl_run_once() the reactor will only run once, then return from
the function.

Stop the reactor with:
...
You create a timeout input source with either of these:
 ...
 //creates a timeout input source and adds it to the reactor for you.
 gwrlsrc * gwrl_set_timeout(gwrl * rl, int64_t ms, bool persist, gwrlevt_cb * cb, void * userdata);
 
 //creates a time input source that you must add to the reactor with gwrl_src_add.
...
Example:
 ...
 int main(int argc, char ** argv) {
     ...
     //create another timeout that isn't persistent - can't be re-armed.
...
Create an interval input source with any of these:

 //creates a repeating interval input source and adds it to the reactor for you.
 gwrlsrc * gwrl_set_interval(gwrl * rl, int64_t ms, gwrlevt_cb * callback, void * userdata);
 
 //create a time input source that you must add to the reactor with gwrl_src_add.
 gwrlsrc * gwrl_src_time_create(int64_t ms, bool repeat, int whence, bool persist, gwrlevt_cb * callback, void * userdata);
...
Create a future date timeout with any of these:
 ...
 //creates a future date timeout and adds it to the reactor for you.
...
File input sources can generate read and write events. The flags
...
Create a file input source with either of these:
 ...
 //creates a file input source and adds it to the reactor for you.
 gwrlsrc * gwrl_set_fd(gwrl * rl, fileid_t fd, uint16_t flags, gwrlevt_cb * cb, void * userdata);
 
 //create a file input source that you must add to the reactor with gwrl_src_add.
...
Any input sources you create that aren't automatically added to the reactor
...
All reactor callbacks use this function prototype:
 ...
 typedef void (gwrlevt_cb)(gwrl * rl, gwrlevt * evt);
...
that contains more information about the event:
 ...
 typedef struct gwrlevt {
     ...
     //gwrlsrc that generated the event, can be NULL. src->fd is the same as fd above.
     struct gwrlsrc * src;
 } gwrlevt;
...
You can create and post events at any time.

Create an event with this: (only I<rl> and I<callback> are required.)
...
This phase calls all callbacks of all gathered events from the previous iteration
...
occur. That time period is used as a timeout value for the IO event backend.
...
get called on every loop iteration for you to poll your own input sources for
any events that need to be posted.
...
There are some cmake compile time options you can use to control internal
...
The number of events to accept from epoll_wait. This does not grow internally
as more input sources are added. This doesn't limit the number of file descriptors
that can be monitiored - only the number of events that can be returned from
epoll_wait with one call. The default is 64. See epoll_wait for more info.
...
The number of events to accept from kevent. This does not grow internally as
more input sources are added. This doesn't limit the number of file descriptors
that can be monitiored - only the number of events that can be returned from
kevent with one call. The default is 64. See kqueue for more info.
...
The number of supported input sources. Currently 2. Refer back to Custom Input Sources
for more information.

=head3 GWRL_REDISPATCH_MAX

The maximum number of times to re-dispatch events that were generated or created
...
When working with a proactor, you still use most of the reactor APIs with the
...
supply the proactor with events.
...
file it means there's nothing left to read, for a pipe means the other end of the
...
actually written - and you can safely free the buffer if you don't need it
anymore. The data that still needs to be written is copied to the write queue
internally.
...
that if your file descriptor is I<not> set to non-blocking, this method will block
...
=head2 Freeing a Proactor

Freeing a proactor only frees the resources used internally but does not
free the file input sources - since those technically belong to the
reactor.

Free a proactor with:

 void gwpr_free(gwpr * pr);

If you're intentions of freeing the proactor are to free all resources, including
the reactor, then you also need to use gwrl_free().
...
GWRL allows you to set memory allocation and freeing functions if needed.
...
All file IO for an input source must happen on the same thread.
...
=head2 Use Thread Specific Data
...
=head1 UNIT AND COVERAGE TESTS

GWRL has some unit tests for testing data structure integridity internaly,
and some tests just for coverage purposes.

Coverage tests are only supported on Mac OS X and Linux, and only
with Makefiles.

=head2 Coverage Tests on Mac OS X

Set the CMAKE_C_COMPILER to "/usr/bin/clang".

Configure and regenerate the Makefile.

Set the CMAKE_BUILD_TYPE to Debug.

Set GWRL_BUILD_COVERAGE on.

Set GWRL_COVERAGE_INTERNAL_ASSERT_VARS on.

Set CMAKE_C_FLAGS_DEBUG to "-g -Wall -fprofile-arcs -ftest-coverage".

Configure and regenerate the Makefile.

Now do this at the commandline:

 make
 ../../scripts/coverage.sh kqueue

When that's all done the generated lcov files are in gwrl/docs/lcov/.
