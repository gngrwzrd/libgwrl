...
for Linux and Mac. On Windows CMake creates a .lib file and .dll.
...
as more input sources are added. The default is 64.
...
more input sources are added. The default is 64.
...
Freeing a proactor only frees the internal resources the proactor requires,
it does not free the file input sources you added to it, or any of the reactor
resources internally.

Freeing a proactor disables all file input sources that were added to with the
proactor API's.

To continue freeing and 
